...
export function SquidGameProfile({ player, protocolToken, token, roundLabels }: any) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#051919] flex flex-col items-center justify-center p-6 text-center overflow-hidden">
        {/* Animated Shapes */}
        <div className="relative w-48 h-48 mb-8 flex items-center justify-center">
          <div className="absolute w-24 h-24 border-4 border-red-600 rounded-full animate-[ping_2s_infinite]" />
          <div className="absolute w-20 h-20 border-4 border-white rotate-45 animate-[pulse_1.5s_infinite]" />
          <div className="w-16 h-16 border-4 border-red-600 flex items-center justify-center">
             <div className="w-8 h-8 bg-red-600 animate-bounce" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-center gap-1">
             <div className="w-2 h-2 bg-red-600 animate-bounce [animation-delay:-0.3s]" />
             <div className="w-2 h-2 bg-red-600 animate-bounce [animation-delay:-0.15s]" />
             <div className="w-2 h-2 bg-red-600 animate-bounce" />
          </div>
          <p className="text-[10px] text-red-900 font-bold uppercase tracking-[0.5em] mt-4">Paradox Protocol v2.0</p>
        </div>

        {/* Scan line effect */}
        <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
           <div className="w-full h-1 bg-red-600/20 shadow-[0_0_15px_rgba(220,38,38,0.5)] absolute animate-[scan_3s_linear_infinite]" />
        </div>
